...
void make_title(const DatalabFrame *frame, const DatalabAppState *state, char *title, size_t title_cap) {
    ...
    if (frame->profile == DATALAB_PROFILE_SKETCH) {
        snprintf(title,
                 title_cap,
                 "DataLab | SKETCH raster=%ux%u logical=%ux%u layers=%u objects=%u rendered=%u unsupported=%u view=%s zoom=%.3fx text=%+d auth=%s/%s pending=%u entry=%u",
                 frame->width,
                 frame->height,
                 frame->logical_width,
                 ...
                 state->raster_viewport.fit_mode ? "fit" : "free",
    ...
    if (frame->profile == DATALAB_PROFILE_IMAGE) {
        snprintf(title,
                 title_cap,
                 "DataLab | IMAGE raster=%ux%u view=%s zoom=%.3fx text=%+d auth=%s/%s pending=%u entry=%u",
                 frame->width,
                 frame->height,
                 state->raster_viewport.fit_mode ? "fit" : "free",
                 (double)state->raster_viewport.viewport.zoom,
